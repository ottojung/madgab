...
### Addendum, same pass (17:12Z), final state

`d4f0b21` succeeded with exit code 0, its fence was reviewed and
**integrated as `ecb42c6`**, its positive control was reproduced by this
coordinator against the integrated tree, and [w-d4f0b2](w-d4f0b2.md) is now
`done` with all six criteria verified. The forbidden shortcut now fails
`cargo test` rather than depending on a coordinator's attention, which is the
pressure this item has been under for eight refuted mechanisms.

`c4e8d70` then finished on its own terms and, told the sweep was integrated,
**kept** it: the staged revert was a hygiene slip in which branch it stood on
while measuring, not an intent to undo the work, and it says so in its own
note above. Its end-state commit `da9c14c` is merged here so the ranks are on
the accumulation branch. This item is **`done`**: criteria 1-7 are met,
membership is non-zero on the default path for six real targets, and the
requested wording's absence is now a selection/pairing fact owned by a new
item, [w-2f7a10](w-2f7a10.md).

Durability reconciliation on the way out, the way the 16:35Z pass prescribed -
`git ls-remote --heads origin` compared against `git for-each-ref refs/heads`,
not against the narrowed remote-tracking refs: **41 local branches, 67 remote,
zero local-only, zero remote-ahead**. Nothing here is single-copy. `main` does
not exist on this host, local or remote.
